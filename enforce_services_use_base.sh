...
for f in src/pages/services/*.astro; do
  ...
  fm="$(awk 'NR==1&&$0=="---"{fmflag=1;next} fmflag&&$0=="---"{fmflag=0;exit} fmflag{print}' "$f")"
  body="$(awk 'NR==1&&$0=="---"{fmflag=1;next} fmflag&&$0=="---"{fmflag=0;next} !fmflag{print}' "$f")"
  ...
  clean_fm="$(printf '%s\n' "$fm" | sed -E '/^[[:space:]]*import[[:space:]]+Base[[:space:]]+from[[:space:]]+/d')"

  tmp="$(mktemp)"
  {
    echo '---'
    [ -n "${clean_fm//[[:space:]]/}" ] && printf '%s\n' "$clean_fm"
    echo 'import Base from "../../layouts/Base.astro";'
    echo '---'
    printf '<Base title="%s">\n' "$title_txt"
    printf '%s\n' "$body"
    echo '</Base>'
