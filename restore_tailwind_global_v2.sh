...
# Kill any preview first (avoids stuck terminals)
pkill -f "astro preview" 2>/dev/null || true

# 1) Install Tailwind + PostCSS deps (safe to re-run)
npm i -D tailwindcss@latest postcss@latest autoprefixer@latest >/dev/null

# 2) Tailwind + PostCSS configs (write directly)
...
# 3) Global styles: tokens + Tailwind layers
mkdir -p src/styles
...
# 4) Single source-of-truth layout that imports global CSS
mkdir -p src/layouts
...
# 5) Normalize services pages to use the layout (idempotent)
python3 - <<'PY'
...
for f in sorted(glob.glob("src/pages/services/*.astro")):
    ...
    for tag in ("Base","SiteLayout","LovableLayout","Layout"):
        src = re.sub(rf'<{tag}\b[^>]*>(.*?)</{tag}>', r'\1', src, flags=re.S|re.I)
...
# 6) Clean build, preview, quick checks
...
echo "== Tailwind tokens present on /services? (look for --tw-) =="
