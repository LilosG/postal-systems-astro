...
for f in files:
    ...
    if os.path.basename(f) == "index.astro":
        ...
        if h1:
            title_txt = html.unescape(re.sub(r'<[^>]+>','', h1.group(1))).strip()
            title = title_txt or humanize(os.path.splitext(os.path.basename(f))[0])
        else:
...
printf "VERIFY FONTS: "
curl -s http://localhost:4321/services | rg -q 'fonts\.googleapis' && echo ok || echo missing
