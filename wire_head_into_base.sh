...
need_fonts="$(rg -n 'fonts\.googleapis|fonts\.gstatic' "$bf" || true)"
need_tokens="$(rg -n '/styles/tokens\.css' "$bf" || true)"
need_css="$(rg -n "$home_css" "$bf" || true)"
need_js="$(rg -n "$home_js" "$bf" || true)"

injected=0
if rg -n "</head>" "$bf" >/dev/null; then
  tmp="$(mktemp)"
  awk -v css="$home_css" -v js="$home_js" -v add_fonts="$([ -z "$need_fonts" ] && echo 1 || echo 0)" \
      ...
      -v add_js="$([ -z "$need_js" ] && echo 1 || echo 0)" '
    BEGIN{ins=0}
    /<\/head>/ && !ins {
      if(add_fonts=="1"){
...
if [ "$injected" -eq 0 ]; then
  tmp="$(mktemp)"
  {
    ...
    echo "  <link href=\"https://fonts.googleapis.com/css2?family=Poppins:wght@600;700;800&family=Inter:wght@400;500;600&display=swap\" rel=\"stylesheet\" />"
    echo "  <link rel=\"stylesheet\" href=\"/styles/tokens.css\" />"
    echo "  <link rel=\"stylesheet\" href=\"$home_css\" />"
    echo "  <script type=\"module\">"
    echo "    if (document.getElementById('root')) { import('$home_js'); }"
    echo "  </script>"
    echo "</head>"
    echo "<body>"
    cat "$bf"
