...
for f in src/pages/services/*.astro; do
  ...
  clean="$(perl -0777 -pe '
    ...
    s{</?body\b[^>]*>}{}igs;
    s{</?SiteLayout\b[^>]*>}{}igs;
    s{</?LovableLayout\b[^>]*>}{}igs;
    s{</?Layout\b[^>]*>}{}igs;
    while (s{<Base\b[^>]*>((?:(?!<Base\b).)*?)</Base>}{$1}igs) {}
  ' <<<"$raw")"

  h1="$(printf '%s\n' "$clean" | sed -n 's/.*<h1[^>]*>\(.*\)<\/h1>.*/\1/p' | sed 's/<[^>]*>//g' | head -n1)"
  base="$(basename "$f")"
  if [ "$base" = "index.astro" ]; then
    page_title="Services | Postal Systems"
  elif [ -n "${h1//[[:space:]]/}" ]; then
    page_title="$(printf '%s' "$h1" | sed 's/^[[:space:]]\+//;s/[[:space:]]\+$//')"
  else
